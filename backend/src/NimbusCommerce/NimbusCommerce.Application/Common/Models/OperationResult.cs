...
/// </summary>
public sealed record OperationResult(bool Succeeded, ErrorCode? Code, string? Message, IReadOnlyList<ValidationFailure> Failures)
{
    public static OperationResult Success() => new(true, null, null, []);

    public static OperationResult NotFound(string message) => new(false, ErrorCode.NotFound, message, []);

    public static OperationResult Conflict(string message) => new(false, ErrorCode.Conflict, message, []);

    public static OperationResult RuleViolation(string message) => new(false, ErrorCode.RuleViolation, message, []);

    public static OperationResult Invalid(IReadOnlyList<ValidationFailure> failures) =>
        new(false, ErrorCode.Validation, "One or more validation errors occurred.", failures);
}

/// <summary>Same shape as OperationResult, carrying a value on success.</summary>
public sealed record OperationResult<T>(bool Succeeded, T? Value, ErrorCode? Code, string? Message, IReadOnlyList<ValidationFailure> Failures)
{
    public static OperationResult<T> Success(T value) => new(true, value, null, null, []);

    public static OperationResult<T> NotFound(string message) => new(false, default, ErrorCode.NotFound, message, []);

    public static OperationResult<T> Conflict(string message) => new(false, default, ErrorCode.Conflict, message, []);

    public static OperationResult<T> RuleViolation(string message) => new(false, default, ErrorCode.RuleViolation, message, []);

    public static OperationResult<T> Invalid(IReadOnlyList<ValidationFailure> failures) =>
        new(false, default, ErrorCode.Validation, "One or more validation errors occurred.", failures);
}
