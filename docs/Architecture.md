...
Implemented: `ApplicationUser`, `RefreshToken`, `ApplicationDbContext`, Identity and JWT-validation configuration, `IIdentityService` / `IdentityService`, `ITokenService` / `TokenService`, `IRefreshTokenStore` / `RefreshTokenStore`, Register, Login, Refresh, and Logout endpoints (`AuthController`), refresh token rotation and reuse detection, the `InitialCreate` database migration, dependency injection wiring for both Application and Infrastructure.

Not yet implemented: `/me` endpoint, seed data, rate limiting, email verification, password reset, MFA, a test project. These are tracked in `project-journal.md`.
